...
Future<void> make(String folder, String text, String filename,
  ...
  try {
    ...
    if (response.statusCode == 200 && response.bodyBytes.isNotEmpty) {
      ...
      print('  ✓  $folder/$filename  (${response.bodyBytes.length ~/ 1024} KB)');
      _generated++;
    } else {
      print('  ✗  $folder/$filename  HTTP ${response.statusCode}');
      _failed++;
    }
    await Future.delayed(const Duration(milliseconds: 280));
  } catch (e) {
    print('  ✗  $folder/$filename  $e');
    _failed++;
  }
}
...
Future<void> main() async {
  ...
  for (final (_, __, phrase, file) in letterSounds) {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  print('\n── Vowel Sounds ────────────────────────────────────');
  await make('vowel_sounds', 'A says Ahh! Like Apple!',     'vowel_a.mp3',         slow: true);
  await make('vowel_sounds', 'E says Ehh! Like Egg!',       'vowel_e.mp3',         slow: true);
  await make('vowel_sounds', 'I says Ihh! Like Ice Cream!', 'vowel_i.mp3',         slow: true);
  await make('vowel_sounds', 'O says Ohh! Like Octopus!',   'vowel_o.mp3',         slow: true);
  await make('vowel_sounds', 'U says Uhh! Like Umbrella!',  'vowel_u.mp3',         slow: true);
  await make('vowel_sounds', 'A, E, I, O, U. These are the vowels!', 'vowels_intro.mp3', slow: false);
  await make('vowel_sounds', 'A says Ahh, like Apple! E says Ehh, like Egg! I says Ihh, like Ice Cream! O says Ohh, like Octopus! U says Uhh, like Umbrella!', 'vowels_all.mp3', slow: true);
  ...
  for (final letter in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('')) {
    await make('alphabet_order', letter, 'name_${ letter.toLowerCase()}.mp3', slow: false);
  }
  await make('alphabet_order', 'A, B, C, D, E, F! Can you put the alphabet in order?', 'intro.mp3', slow: false);
  await make('alphabet_order', 'Yes! That is correct! Keep going!',  'correct.mp3',  slow: false);
  await make('alphabet_order', 'Oops! Try the next letter in order!', 'wrong.mp3',   slow: false);
  await make('alphabet_order', 'Amazing! You know the whole alphabet!', 'win.mp3',   slow: false);
  ...
  await make('sound_match', 'What letter does it start with?', 'question.mp3',  slow: false);
  await make('sound_match', 'Tap to hear the word!',           'tap_to_hear.mp3', slow: false);
  await make('sound_match', 'Tap the correct starting letter!','tap_letter.mp3',  slow: false);

  // ══════════════════════════════════════════════════════════════════════════
  // 5. MEMORY FLIP GAME
  // ══════════════════════════════════════════════════════════════════════════
  print('\n── Memory Flip ─────────────────────────────────────');
  await make('memory_flip', 'Match the letter to its picture!',           'intro.mp3',       slow: false);
  await make('memory_flip', 'Find the letter that matches the picture!',   'instruction.mp3', slow: false);
  await make('memory_flip', 'Great match!',                                'match.mp3',       slow: false);
  await make('memory_flip', 'All pairs found! Amazing job!',               'win.mp3',         slow: false);
  // Letter names reused for card flips
  for (final letter in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('')) {
    await make('memory_flip', letter, 'name_${letter.toLowerCase()}.mp3', slow: false);
  }
  ...
  await make('phonics_quiz', 'What sound does it start with?', 'question.mp3',  slow: false);
  ...
  final wbWords = ['Cat','Dog','Sun','Ape','Fin','Rat','Hut','Lip','Mop'];
  ...
  await make('say_it_right', 'Tap the mic and say the word!',           'instruction.mp3',    slow: false);
  await make('say_it_right', 'Listening... speak now!',                  'listening.mp3',      slow: false);
  await make('say_it_right', 'Great job! You said it correctly!',        'correct.mp3',        slow: false);
  await make('say_it_right', 'Good try! Listen and try again.',          'try_again.mp3',      slow: false);
  await make('say_it_right', 'Practice done! You did amazing!',          'done.mp3',           slow: false);
  ...
  final rhymeWords = ['Cat','Bat','Bee','Tree','Moon','Spoon','Pig','Big',
                      'Hat','Mat','Bug','Mug','Star','Car','Frog','Log',
                      'House','Mouse','Cake','Lake','Fox','Box','Balloon'];
  for (final w in rhymeWords) {
    await make('rhyming_words', w, 'word_${w.toLowerCase()}.mp3', slow: true);
  }
  await make('rhyming_words', 'Which word rhymes with this?',         'question.mp3',  slow: false);
  await make('rhyming_words', 'Tap the word that rhymes!',            'instruction.mp3', slow: false);
  await make('rhyming_words', 'Cat, Bat, Hat! They rhyme!',           'intro.mp3',     slow: false);
  await make('rhyming_words', 'Rhyming words end with the same sound!', 'explain.mp3', slow: false);
  ...
  for (final sub in phonicsDir.listSync().whereType<Directory>()) {
    ...
    print('  ${sub.path.split(Platform.pathSeparator).last.padRight(20)} $count files');
  }
  print('──────────────────────────────────────────────────');
  print('✅  Generated : $_generated');
  if (_failed > 0) print('❌  Failed    : $_failed');
  print('══════════════════════════════════════════════════\n');
}
