...
Uri _ttsUrl(String text, {bool slow = false}) {
  return Uri.https('translate.google.com', '/translate_tts', {
    'ie': 'UTF-8', 'q': text, 'tl': 'en',
    'ttsspeed': slow ? '0.5' : '0.9', 'client': 'tw-ob',
  });
}
...
Future<void> make(String filename, String text, {bool slow = false}) async {
  ...
  try {
    ...
    if (res.statusCode == 200 && res.bodyBytes.isNotEmpty) {
      ...
      print('  ✓  $filename  (${res.bodyBytes.length ~/ 1024} KB)');
      _generated++;
    } else {
      print('  ✗  $filename  HTTP ${res.statusCode}');
    }
...
Future<void> main() async {
  ...
  print('\n── Game Intros ──');
  await make('intro_sound_match.mp3',  'Let us play Sound Match! Listen carefully!', slow: false);
  await make('intro_memory.mp3',       'Memory Flip time! Find the matching pairs!', slow: false);
  await make('intro_quiz.mp3',         'Phonics Quiz! Name the sound you hear!', slow: false);
  await make('intro_word_builder.mp3', 'Word Builder! Spell the missing letter!', slow: false);
  await make('intro_alphabet.mp3',     'Alphabet Order! Put the letters in order!', slow: false);
  await make('intro_rhyming.mp3',      'Rhyming Words! Find the word that rhymes!', slow: false);
  await make('intro_voice.mp3',        'Say It Right! Speak the word clearly!', slow: false);

  // ── MILESTONE / ACHIEVEMENT ───────────────────────────────────────────
  print('\n── Milestones ──');
  await make('level_up.mp3',           'Level up! You are getting better and better!');
  await make('achievement_first.mp3',  'You learned your first letter! Great start!');
  ...
  await make('achievement_26.mp3',     'Alphabet Master! All twenty six letters learned!');
  await make('achievement_xp.mp3',     'One hundred XP earned! You are an XP hunter!');
  await make('achievement_streak.mp3', 'Three day streak! Keep coming back to learn!');

  // ── SPECIFIC WRONG ANSWER EXPLANATIONS ────────────────────────────────
  print('\n── Wrong Answer Explanations ──');
  // Sound Match
  await make('wrong_sound_match.mp3', 'Listen again! The word starts with a different sound!', slow: true);
  // Phonics Quiz
  await make('wrong_quiz.mp3',        'Not quite! Think about the first sound of the word!', slow: true);
  // Word Builder
  await make('wrong_word.mp3',        'Try again! Think about what letter is missing!', slow: true);
  // Memory Flip
  await make('wrong_memory.mp3',      'Not a match! Try to remember where the letters are!', slow: true);
  // Alphabet Order
  await make('wrong_alphabet.mp3',    'Not the right order! Think about which letter comes next!', slow: true);
  // Rhyming
  await make('wrong_rhyming.mp3',     'That does not rhyme! Listen for the same ending sound!', slow: true);

  _client.close();
  print('\n══════════════════════════════');
  print('✅  Generated: $_generated files');
  print('══════════════════════════════\n');
}
